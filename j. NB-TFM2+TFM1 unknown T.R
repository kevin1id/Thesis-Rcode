...
constraint_time_star <- function(time_star){
  return((time_star[1] > 5 & time_star[1] < 40) & # 45 
           ((time_star[2] > (time_star[1] + 40)) & time_star[2] < 135))
}
...
liko<-function(nob,om,alpha,beta,yt,r,con,delta,time){
  liko=0
  ...
  { 
    ...
    liko= liko+lgamma(yt[t]+r) - lgamma(r) - r*log(1+lam_o[t]) + yt[t]*log(lam_o[t])-yt[t]*log(1+lam_o[t])
  }
  return(liko)
}
...
for (isi in 1:nosim){
  ...
  for (i in 1:M){
    ...
    lik1=liko(nob,om,alpha,beta,yt,r,con,delta,time)
    lik2=liko(nob,star[1],star[2],star[3],yt,r,con,delta,time)
    ...
    lik3=liko(nob,om,alpha,beta,yt,r,con,delta,time) + dgamma(r, shape = c_1, rate = c_2, log = TRUE)    # (c_1-1)*log(r)-c_2*r
    lik4=liko(nob,om,alpha,beta,yt,star2,con,delta,time) + dgamma(star2, shape = c_1, rate = c_2, log = TRUE) # (c_1-1)*log(star2)-c_2*star2
    ...
    if(i  <= ind)
    {
      repeat 
      {
        ...
        # star1[4]	= delta[2] +rnorm(1,0,1)*step_delta
    ...
    lik5=liko(nob,om,alpha,beta,yt,r,con,delta,time) + 
      sum(dgamma(con, shape = b_1, rate = b_2, log = TRUE)) + 
      sum(dbeta(delta, shape1 = a_1, shape2 = a_2, log = TRUE))
    lik6=liko(nob,om,alpha,beta,yt,r,star1[1:2],star1[3],time) + 
    ...
    { 
      ...
      time_star[2]= round((time_star[1]+40)+rnorm(1,0,1)*step_time[2])
      if(constraint_time_star(time_star)){break} 
    }
    
    lik9=liko(nob,om,alpha,beta,yt,r,con,delta,time)
    lik10=liko(nob,om,alpha,beta,yt,r,con,delta,time_star)
    ...
    draws[i,] = c(om ,alpha,beta,r,con,delta, time)
    ...
    if(i  > ind){
      for (t in 2:nob)
      {
        ...
        m[t]=r*lam_o[t]
        v[t]=r*lam_o[t] + r*lam_o[t]^2
        
        res[t]=(yt[t]-m[t])/sqrt(v[t])
        
        yt_pred[t]	  = rnbinom(1, size = r, mu = r*lam_o[t])
      }
    ...
    # for DIC
    lik11 <- liko(nob,om,alpha,beta,yt,r,con,delta,time)
  ...
  liko12 <- liko(nob,om_mean,alpha_mean,beta_mean,yt,r_mean,con_mean,delta_mean,time_mode)
  ...
  names		 = c(list(bquote(alpha[0])),list(bquote(alpha[1])),list(expression(beta)), 
              list(expression(r)),list(bquote(omega[0][2])),list(bquote(omega[0][1])), 
              list(bquote(delta[1])))
  if (isi==1)
  {
...
}
{
  ...
  rownames(result) <- c("omega", "alpha", "beta", "r", "c1", "c2", "delta")
  print(result)
  result_T = data.frame(mean = c(T1_mode, T2_mode))
  rownames(result_T) <- c("T1", "T2")
  print(result_T)
  D=apply(DIC,2,mean)
  print(D)
  acc = c(acceptrate = count / M,
  ...
  print(round(pnorm(Geweke.Diagnostic(draws[MCMC, 1:length(names)])), 4))
  
  # Inefficiency factors
  compute_inefficiency_factor_autocorr <- function(samples, max_lag = 100) {
    acf_values <- acf(samples, plot = FALSE, lag.max = max_lag)$acf[-1]
    IF <- 1 + 2 * sum(acf_values)
    return(IF)
  }
  ineff_factors = sapply(1:length(names), function(j) compute_inefficiency_factor_autocorr(draws[MCMC, j]))
  print(round(ineff_factors, 4))
  }
  
  Geweke.Diagnostic(draws[MCMC,1:length(names)]) %>%  pnorm(., 0, 1) %>% round(4)
  # 0.8169 0.1435 0.3486 0.7492 0.5670 0.7117 0.6016
  sapply(1:length(names), function(j) compute_inefficiency_factor_autocorr(draws[MCMC,j])) %>%  round(4)
  # 4.3561 7.8853 7.8656 3.0522 1.8414 4.3035 3.6772
  
  # Traceplot and ACF plot
  png("traceplot_acf_NB_2B_2A.png", width = 18, height = 10, units = "in", res = 300)
  ...
  saveRDS(newdata, "in_sample_NB_2B_2A.rds")
  
  intervention <- data.frame(xint = c(r8[1]-4, r9[1]-4),  # 39 86
                             yint = c("March 2016", "February 2020"),
                             type = "Intervention")
  ...
  plot= newdata %>% pivot_longer(cols = c("Observed","Predicted"),names_to="var",values_to="val") %>%
    ...
    annotate("text", x = intervention$xint, y = c(13, 13),
             label = intervention$yint, hjust = 1.1, size = 5) +
    scale_color_manual(
      name   = " ",
...
png("in-sample_NB_2B_2A.png", width = 300, height = 200, unit="mm", res = 300) #350
plot
dev.off()
...
# Standardized residual line plot
plot(ts(Mean_resid), main = "Standardized Residual Plot", ylab = "", xlab = "", lwd = 2, xaxt = 'n')
axis(1, at = c(1,  24-4,  48-4, 72-4,  96-4,  120-4), labels = c("2013","2015","2017",  "2019", "2021",  "2023"))
abline(h = 0, col = "red", lwd = 2)

# ACF of standardized residuals
acf(Mean_resid, main = "ACF of Standardized Residual", lwd = 2)

# ACF of standardized squared residuals
acf(Mean_resid_square, main = "ACF of Standardized Squared Residual", lwd = 2)
