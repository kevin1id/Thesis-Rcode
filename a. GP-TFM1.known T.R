...
data <- read.csv("C:/Users/ACER/Desktop/working2/DRUGS transfer function/R Codes. Kevin/Dipolog crimes.csv", header = TRUE)
...
for (isi in 1:nosim)
{
  ...
  for (i in 1:M){
    ...
    draws[i,] = c(om ,alpha,beta,psi,con)
 ...
 # time_mode=getmode(draws[MCMC,6])
  ...
  for (t in 2:nob)
  { 
    Pttt=c(rep(0,time-1),1,rep(0,nob-time))
  ...
  ############################# PLOT
  names		 = c(list(expression(omega)),list(expression(alpha)),list(expression(beta)), list(expression(psi)),"c")
  if (isi==1) 
  {
...
r5=apply(S5,2,mean)
#r6=apply(S6,2,mean)
result=round(rbind(r1,r2,r3,r4,r5),4)
