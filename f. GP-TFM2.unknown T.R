...
for (isi in 1:nosim)
{
  ...
  for (i in 1:M){
    ...
    lik7<-liko(nob,yt,om,alpha,beta,psi,con,delta,time)
    lik8<-liko(nob,yt,om,alpha,beta,psi,con,delta,time_star)
  ...
  ############################# PLOT
  names		 = c(list(expression(omega)),list(expression(alpha)),list(expression(beta)), list(expression(psi)),"c" ,list(expression(delta)), "Time")
  if (isi==1) 
  {
...
# Extract Time from result (ensure it's numeric)
time_value <- as.numeric(result["Time", "mean"])  

plot <- ggplot(DS1pred_y_res, aes(x = year, y = data)) + 
  geom_line(aes(color = value, linetype = value), size=0.8) + 
  scale_color_manual(values = c("red", "blue")) +     
  scale_x_continuous(
    breaks = c(1,12,24,36,48,60,72,84,96,108,120,132,140),
  ...
  # Add vertical line dynamically for "Time"
  geom_vline(xintercept = time_value, linetype="dashed", color="black", size=1) +
  
  # Add label dynamically at the top of the plot
  annotate("text", x = time_value, y = max(DS1pred_y_res$data, na.rm=TRUE), 
           label = paste("Time =", round(time_value)), vjust = -0.5, size = 5, fontface = "bold")
