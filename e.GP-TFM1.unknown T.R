...
liko<-function(nob,yt,om,alpha,beta,psi,con,time){
  liko=0
...
for (isi in 1:nosim)
{
  ...
  for (i in 1:M){
    ...
    lik1<-liko(nob,yt,om,alpha,beta,psi,con,time)
    lik2<-liko(nob,yt,star[1],star[2],star[3],psi,con,time)
    ...
    lik3<-liko(nob,yt,om,alpha,beta,psi,con,time)
    lik4<-liko(nob,yt,om,alpha,beta,psi_star,con,time)
    ...
    lik5<-liko(nob,yt,om,alpha,beta,psi,con,time)+(b_1-1)*log(con)-b_2*con
    lik6<-liko(nob,yt,om,alpha,beta,psi,star1,time)+(b_1-1)*log(star1)-b_2*star1
    ...
    lik7<-liko(nob,yt,om,alpha,beta,psi,con,time)
    lik8<-liko(nob,yt,om,alpha,beta,psi,con,time_star)
    ...
    draws[i,] = c(om ,alpha,beta,psi,con,time)
  ...
  ############################# PLOT
  names		 = c(list(expression(omega)),list(expression(alpha)),list(expression(beta)), list(expression(psi)),"c", "Time" )
  if (isi==1) 
  {
...
Time_value <- result["Time", "mean"]  # Extract the computed Time
...
plot = ggplot(DS1pred_y_res, aes(x = year, y = data)) + 
  ...
  xlab("") + ylab("") + ggtitle("Illegal Drugs") +
  ...
  # Add vertical line at the computed Time value
  geom_vline(xintercept = Time_value, linetype = "dashed", color = "black", linewidth = 1) +
  
  # Add label at the top of the vertical line
  annotate("text", x = Time_value, y = max(DS1pred_y_res$data, na.rm = TRUE), 
           label = paste("Time =", round(Time_value)), vjust = -0.5, hjust = 1, size = 6, color = "black")
