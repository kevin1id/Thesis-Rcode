...
for (isi in 1:nosim)
{
  ...
  for (i in 1:M){
   ...
   lik2=liko(nob,yt,star[1],star[2],star[3],r,con,delta,time)
    ...
    lik9=liko(nob,yt,om,alpha,beta,r,con,delta,time)
    lik10=liko(nob,yt,om,alpha,beta,r,con,delta,time_star)
  ...
  for (t in 2:nob)
  { 
    Pttt=c(rep(0,time_mode-1),1,rep(1,nob-time_mode))
    if(t<time_mode){
      ...
      lam_o[t] = om_mean+ alpha_mean*yt[t-1] + beta_mean*lam_o[t-1]+con_mean*((delta_mean)^(t-time_mode))*Pttt[t]
      lik8= lik8-2*lgamma(yt[t]+r_mean)+2*lgamma(r_mean) +2*lgamma(yt[t]+1)+2*r_mean*log(1+lam_o[t]) -2*yt[t]*log(lam_o[t])+2*yt[t]*log(1+lam_o[t])   
    }
  }
  ...
  ############################# PLOT
  names		 = c(list(expression(omega)),list(expression(alpha)),list(expression(beta)), "r","c" ,list(expression(delta)),"Time")
  if (isi==1) 
  {
...
# Extract the dynamically computed "Time" value
Time_value <- result["Time", "mean"]  # Get the computed Time from the result matrix

# Update the ggplot to include a vertical line for the computed Time
plot <- newdata %>% 
  pivot_longer(cols = 'Observed':'Predicted', names_to="var", values_to="val") %>%
  ggplot(aes(x=year, y=val, color=var, group=var, linetype=var)) +
  geom_line(linewidth=0.75) +  # Use linewidth instead of size for ggplot2 3.4+
  ...
  # Add a vertical line at the computed Time value
  geom_vline(xintercept = Time_value, linetype="dashed", color="black", linewidth=1) +
  
  # Add label for the computed Time value
  annotate("text", x = Time_value, y = max(newdata$Observed, na.rm=TRUE), 
...
plot+theme(plot.tag.position = c(0.30,0.95))
